...
class InternVL3Points:
    def __init__(self, model_id: str, device: str = "cuda", max_new_tokens: int = 256):
        ...
        # ✅ 4-bit safe loading (no .to())
        self.model = AutoModel.from_pretrained(
            model_id,
            trust_remote_code=True,
            low_cpu_mem_usage=True,
            device_map="auto",   # Let Accelerate handle GPU mapping
            load_in_4bit=True,   # Quantized loading mode
        ).eval()
        ...
        # ✅ Patch for models missing .generate()
        if not hasattr(self.model, "generate") and hasattr(self.model.language_model, "generate"):
            self.model.generate = self.model.language_model.generate
            print("⚙️ Patched missing .generate() method.")
    ...
    def infer_points(self, rgb_crop: np.ndarray, poly_xy: list[tuple[int, int]]) -> dict | None:
        ...
        # ✅ tensor on correct device + dtype of model
        pixel_values = tfm(img).unsqueeze(0).to(
